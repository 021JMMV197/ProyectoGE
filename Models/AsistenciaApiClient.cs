...
namespace ProyectoGE.Models
{
    ...
    public class AsistenciaUpdateDto : AsistenciaCreateDto
    {
        public int IdAsistencia { get; set; }
    ...
    public class AsistenciaApiClient
    {
        ...
        public async Task<AsistenciaView[]> ListAsync(int? idEmpleado = null, DateTime? desde = null, DateTime? hasta = null)
        {
            using (var http = NewClient())
            {
                var url = $"{_baseUrl}api/asistencia";
                var sep = "?";
                ...
                return JsonConvert.DeserializeObject<AsistenciaView[]>(json);
            }
        }

        public async Task<int> CreateAsync(AsistenciaCreateDto d)
        {
            using (var http = NewClient())
            {
                var payload = JsonConvert.SerializeObject(d);
                var resp = await http.PostAsync($"{_baseUrl}api/asistencia",
        ...
        public async Task<bool> UpdateAsync(AsistenciaUpdateDto d)
        {
            using (var http = NewClient())
            {
                var payload = JsonConvert.SerializeObject(d);
                var resp = await http.PutAsync($"{_baseUrl}api/asistencia/{d.IdAsistencia}",
                    new StringContent(payload, Encoding.UTF8, "application/json"));
        ...
        public async Task<bool> DeleteAsync(int id)
        {
            using (var http = NewClient())
            {
                var resp = await http.DeleteAsync($"{_baseUrl}api/asistencia/{id}");
                if (resp.StatusCode == System.Net.HttpStatusCode.NoContent) return true;
                if (resp.StatusCode == System.Net.HttpStatusCode.NotFound) return false;
                resp.EnsureSuccessStatusCode();
                return true;
